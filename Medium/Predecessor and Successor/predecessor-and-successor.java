...
public static void findPreSuc(Node root, int key)
{
    ...
    */
    ...
    findPre(root, key, ans1);
    findSuc(root, key, ans2);
}
static void  findPre(Node root, int key, int ans1){
    if(root == null) return;
    if(root.data<key){
         if(ans1<root.data){
            ans1 = root.data;
        ...
        findPre(root.left, key, ans1);
    }
}
static void findSuc(Node root, int key, int ans2){
    if(root== null) return;
   if(root.data>key) {
        if(ans2>root.data){
           ans2 = root.data;
